...
window.COMPLETENESS_MOD_LOADED = true;
...
export class CompletenessModule {
        ...
        if (datasetSelect) {
            datasetSelect.addEventListener('change', async (e) => {
                ...
                try {
                    ...
                    this.buildPeriodPicker(info.periodType || 'Monthly');
                    this.initDataElements(info.dataElements || []);
     ...
     */
    selectRecentPeriods() {
        const periodType = this.datasetInfo?.periodType || 'Monthly';
        const periods = generatePeriods(periodType, 50);
    ...
    openDetail(ouId) {
        ...
        (this.datasetInfo?.dataElements || []).forEach(de => {
        ...
        const requiredIds = (this.selectedElements && this.selectedElements.size > 0)
            ...
            : (this.datasetInfo?.dataElements || []).map(de => de.id);
    ...
    async _bulkAction(action) {
        ...
        try {
            const request = {
                profile_id: this.app.currentProfile.id,
